...
// Openview — a Mac PDF AI viewer. Pure-AppKit, NSDocument-based rewrite (see files/migration_appkit.md).
...
let package = Package(
    ...
    targets: [
        ...
        .target(name: "OpenviewKit", swiftSettings: [.swiftLanguageMode(.v5)]),
        ...
        .executableTarget(
            name: "Openview",
            dependencies: ["OpenviewKit"],
            swiftSettings: [.swiftLanguageMode(.v5)]
        ),
        // Coordinate-PoC driver (gated, headless): selftest | gen | truth | convert | render | e2e.
        // Drives the SAME live OpenviewKit.CoordinateAdapter the app uses, so the #7 gate measures the
        // shipping transform — not a copy. Ported from the prior build (import CoordPoC → OpenviewKit).
        .executableTarget(
            name: "coordcli",
            dependencies: ["OpenviewKit"],
            swiftSettings: [.swiftLanguageMode(.v5)]
